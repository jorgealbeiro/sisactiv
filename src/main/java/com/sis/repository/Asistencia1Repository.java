...
//	Collection<Object> obtenerPersonas();
	...
	@Query(value = "SELECT categoria_actividad.nombre as nombreCategoria ,adulto_mayor.nombre, adulto_mayor.apellido,  adulto_mayor.genero, actividad.fecha from  categoria_actividad join actividad on categoria_actividad.id_categoria=actividad.categoria_id "
			+ "			join asistencia1 on actividad.id=asistencia1.id_actividad join adulto_mayor on asistencia1.cedula_adulto=adulto_mayor.cedula "
			+ "            where categoria_actividad.id_categoria =(:id) ", nativeQuery = true)
